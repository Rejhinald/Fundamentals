...
func (c UserController) ManageAccess() revel.Result {
	...
	// Get user info upfront since we need it in both flows
	...
	if payload.EnableAccess {
		// Check if user exists in company first
		companyUser, err := GetCompanyUser(companyID, user.UserID)
		...
		if payload.SendEmail {
			...
			// REMOVED the duplicate user lookup since we already have the user from the start
			recipient = append(recipient, mail.Recipient{
				Name:           user.FirstName + " " + user.LastName,
				Email:          user.Email,
				InviteUserLink: frontendUrl + "/companies/" + company.CompanyID,
